...
 In cloud environments, multiple users share the same physical server. It leads to compromission of data, as data from one user can potentially impact others on the same server.

 Mitigation includes implementation of strong isolation mechanisms such as hypervisor protection, secure multi-tenancy. also, by performing regular vulnerability scanning, and patching security gaps.
...
   - Monitor user activities for suspicious behaviour
...
   - Use multiple servers, data centres, or cloud providers

   - Regularly test failover procedures

Weak Authentication and Encryption Practices

 Weak authentication and encryption can expose cloud system and data, this can result to unauthorised access and data leaks.
...
  It is a lightweight alternative, this technology encapsulate app with their OS environment.
...
  Type 1 (Bare Metal): In this the hypervisor runs directly on Hardware e.g. Hyper-V, XenServer, ESXi.

  Type 2 (Hosted): In this the hypervisor operates within a standard OS e.g. VirtualBox, VMware.
...
  Docker, Kubernetes, Red Hat, OpenShift are popular containerization platforms. These are the application that revalorized the deployment in cloud environment.
...
 - Scalability: In this architecture the services can be scaled independently based on demand.
...
 It is the backbone of modern organizations. It comprises of hardware, software, services, and facilities for network support and management.

Physical separation

 This is often referred to as "Air Gapping". It isolates a system by physically disconnecting it for all networks. Physical separation is one of the most secure methods of security, but it is still vulnerable to sophisticated attacks.

Logical Separation

 This establishes boundaries within a network to restrict access to a certain area. It is implemented by using Firewalls, VLANs, and network devices.

Comparison

 Physical Separation (Air Gapping): It is highly secure as it provides complete isolation.

 Logical Separation: It is more flexible and easier to implement. It is less secure if not configured properly.

### Software-defined Network (SDN)

Software-defined Network

 Software-Defined Networking (SDN) is a transformative approach to network management that decouples the network control plane from the data forwarding plane. This separation allows for more dynamic and programmable network configurations, making networks more flexible, efficient, and easier to manage. This management is done with a centralized server that decides how data moves through the network.
...
Infrastructure as Coad (IaC)

 It is a modern approach to It Infrastructure management. It automates the provisioning and management through code. This is primarily used in DevOps and with cloud computing.

IaC Method

 In this the developers and ops teams manage infrastructure through code. These codes files are versioned, tested and audited. High level languages like YAML, JASON or domain specific languages are used e.g. HCL.  Idempotence methods are used to ensure the identical environments. (Idempotence refers to a function or an operation that can be repeated without changing the outcome.)
...
  - Efficiency and control: centralization provides high resource control and efficient resource allocation as it has status of whole infrastructure.
...
 - Scalability: It can easily scale with organization growth by adding new nodes.
...
 It is central component connecting IoT devices. It collects, processes, analyses data, and sends commands. It can either be a physical device or software platform.
...
 Poorly Configured Network Services: Devices may have vulnerabilities due to open ports, unencrypted communications. The unnecessary services can increase attack surface. Keeping IoT devices on a separate network is recommended.
...
 Programmable logic Controllers (PLCs): It is used to control specific processes such as assembly lines and factories.
...
 - Challenges in Patching: Updates face operational constraints; OTA updates demand meticulous planning and security measures. It also includes Over-the-Air (OTA) Updates which means Patches are delivered and installed remotely.